...
int main() {
  ...
  { loadImage(); firmware(); NW_Device d; d.begin(0x41, "Apis", 2);
    ...
    printf("[handshake] takeReading(chip0): ok=%d counter=%u newReading=%d ctrl=0x%02X fault='%s' transactions=%u\n",
       ok, d.readCounter(), d.newReading(), Wire.image[0x41], kindText(d.report()), Wire.transactions - tx);
  ...
  { loadImage(); firmware(); NW_Device d; d.begin(0x41, "Apis", 2);
    Wire.beforeRead = [](TwoWire& w, uint8_t) { w.image[0x40] = 0x83; w.image[0x47] = 0x02; };   // ready | chip0 fault | pan; chip 0 timeout
    d.takeReading(0x01);
    ...
    Wire.beforeRead = [](TwoWire& w, uint8_t) { w.image[0x40] = 0x83; w.image[0x47] = 0x01; };   // chip 0 no acknowledge
    d.takeReading(0x01); printf("[report] chip0 no-ack, chip0 selected: batchFaulted=%d '%s'\n", d.batchFaulted(), kindText(d.report()));
    d.writeBatch(4); printf("[report] writeBatch(4): batchFaulted=%d word=%u\n", d.batchFaulted(), Wire.image[0x44] | (Wire.image[0x45] << 8));
    Wire.beforeRead = [](TwoWire& w, uint8_t) { w.image[0x40] = 0x85; w.image[0x47] = 0x21; };   // chip 1 no-ack
    ...
    Wire.beforeRead = [](TwoWire& w, uint8_t) { w.image[0x40] = 0x01; w.image[0x47] = 0xE6; };   // unit: reset since configured
    ...
    Wire.beforeRead = [](TwoWire& w, uint8_t) { w.image[0x40] = 0x01; w.image[0x47] = 0x11; };   // chip 0 kind 17
  ...
  { loadImage(); NW_Device d; d.begin(0x41, "Apis", 2); for (int i = 0; i < 128; i++) Wire.image[i] = i;
    uint8_t buf[40] = {0}; unsigned tx = Wire.transactions; bool ok = d.readBytes(0x40, buf, 40);
    printf("[readBytes] 40 bytes from 0x40: ok=%d transactions=%u first=0x%02X last=0x%02X\n", ok, Wire.transactions - tx, buf[0], buf[39]); }
  ...
  { loadImage(); firmware(); NW_Device d; d.begin(0x41, "Apis", 2); d.takeReading(0x01);
    int commits = 0; Wire.image[0x48] = 0x11;
    Wire.afterRequest = [&](TwoWire& w) { if (commits++ == 0) { w.image[0x48] = 0xAA; w.bumpCounter(); } };
    uint16_t captured = d.readCounter(); uint8_t b = 0; d.readBytes(0x48, &b, 1);
    ...
    commits = 0; Wire.image[0x48] = 0x11; d.captureReading(); unsigned tx = Wire.transactions;
    bool ok = d.readData(0x48, &b, 1);
    ...
    Wire.afterRequest = [](TwoWire& w) { w.image[0x48]++; w.bumpCounter(); };
    d.captureReading(); tx = Wire.transactions; ok = d.readData(0x48, &b, 1);
  ...
  { loadImage(); NW_Device d; d.begin(0x41, "Apis", 2); d.writeBatch(300); d.writeConfig(0x03); d.sleep(); d.setI2CAddress(0x45);
    printf("[registers] batch=%u config=0x%02X ctrl=0x%02X addr=0x%02X readConfig=0x%02X\n",
       Wire.image[0x44] | (Wire.image[0x45] << 8), Wire.image[0x46], Wire.image[0x41], Wire.image[0x1F], d.readConfig()); }
